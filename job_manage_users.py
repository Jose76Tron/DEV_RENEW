...
class ManageUsers:
    ...
    def get_user_count(self):
        response = call_api_get(
            "https://graph.microsoft.com/v1.0/users", self.access_token
        )
    ...
    def get_random_user(self):
        ...
        return random.choice(
            [item.get("userPrincipalName") for item in response.json()["value"]]
        )
    ...
    def add_user(self, recursive_count=0):
        ...
        response = call_api_post(
            "https://graph.microsoft.com/v1.0/users", data, self.access_token
        )
        if response.status_code == 201:
            ...
            if recursive_count < 5:
                self.add_user(
                    recursive_count + 1
                )  # Recursively call this function until its success

    # Delete a user by id or userPrincipalName
    def delete_user_by_id(self, id, recursive_count=0):
        response = call_api_delete(
            f"https://graph.microsoft.com/v1.0/users/{id}", self.access_token
        )
        if response.status_code == 204:
            ...
            print(
                "{}: {} {} {} {}".format(
                    "Error", "Delete user", id, response.status_code, response.text
                )
            )
            if recursive_count < 5:
    ...
    def check_user_role(self, id):
        ...
        # if json value not exists, return False
    ...
    def get_random_domain(self):
        response = call_api_get(
            "https://graph.microsoft.com/v1.0/domains", self.access_token
        )
    ...
    def run(self):
        ...
        if (
            target_user_count >= 0.5 * user_count_midpoint
            and target_user_count <= 1.5 * user_count_midpoint
        ):
