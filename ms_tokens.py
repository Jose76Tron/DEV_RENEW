...
class MSTokens:
    ...
    def get_access_token(self):
        ...
        if response.status_code == 200:
            ...
            print(
                "{}: {} {} {}".format(
                    "Error", "Get access token", response.status_code, response.text
                )
            )
    ...
    def save_tokens_to_file(self, data):
        # Create a backup
        tokens_backup_file_path = os.path.join(
            self.app_folder,
            self.tokens_file_name[:-4] + "-" + str(int(time.time())) + ".txt",
        )
